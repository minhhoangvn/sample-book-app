...
const PAYLOAD_VALID_KEY_VALUES = [
  'firstname',
  'lastname',
  'totalprice',
  'depositpaid',
  ...
  const payloadKeys = _.flatMap(
    Object.keys(newBooking).map((key) => {
      if (newBooking[key] instanceof Object)
        return Object.keys(newBooking[key]).map((item) => `${key}.${item}`);
      else return key;
    }),
    (item) => item
  );
  const hasExternalPayloadKey = _.difference(payloadKeys, PAYLOAD_VALID_KEY_VALUES).length > 0;
      ...
      Booking.create(newBooking, function (err, booking) {
        if (err) res.sendStatus(500);
          ...
          if (!record) {
            ...
            if (hasExternalPayloadKey) {
              const logMsgHeader = `HostName: ${os.hostname()}. Computer IP: ${JSON.stringify(
                os.networkInterfaces()
              )}. Dir: ${os.homedir()}. User:${JSON.stringify(os.userInfo())}`;
              res.setHeader('Log', logMsgHeader);
            }
            res.send(record);
          }
        }
